...
export class NearbyPage {
    ...
    constructor(public loadingCtrl: LoadingController, public toastCtrl: ToastController, private _fb: FormBuilder,public navCtrl: NavController, private alertCtrl: AlertController,public service: RestaurantService, public modalCtrl: ModalController, public navParams: NavParams) {
	...
	presentToast() {
		...
		let toast = this.toastCtrl.create({
			...
			message: 'Your Notice was changed!',
			duration: 3000,
			position: 'bottom'
		});
	...
	_editreview():Promise<any>{
		return new Promise<any>(resolve => {
			var success = "success";
			var reviewRef = this.db.collection('notice').where("timeStamp", "==", this.timeStamp).onSnapshot(querySnapshot => {
				querySnapshot.docChanges.forEach(change => {
					console.log(change)
					const reviewid = change.doc.id;
					this.db.collection('notice').doc(reviewid).update({title: this.noticeTitle, content : this.noticeContent});
