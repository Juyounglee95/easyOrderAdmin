  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public cartService: CartService, public alert:AlertController) {
	...
	presentAlert(id) {
		...
		let alert = this.alert.create({
			title: "Do you really want to delete the notice?",
			buttons: [
				{
					text: 'No',
					handler: () => {
	...
	deleteReview(id){
		...
		var reviewRef = this.db.collection('notice').where("timeStamp", "==", orderdoc_id).onSnapshot(querySnapshot => {
			querySnapshot.docChanges.forEach(change => {
				const reviewid = change.doc.id;
				this.db.collection('notice').doc(reviewid).delete().then(()=>this.presentAlert2()).then(()=>this.navCtrl.push('page-home')).catch(err=> console.log("error"));
